...
- New field in EVC document ``error_status`` which is a dictionary with content explaining an error that happened when installing or deleting ``current_path`` or ``failover_path`` or ``intra_switch_path``.
