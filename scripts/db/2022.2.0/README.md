## MEF-ELine's migration scripts for Kytos version 2022.2.0

This folder contains MEF-ELine's related scripts.

### Data migration from `storehouse` to MongoDB

[`000_storehouse_to_mongo.py`](./000_storehouse_to_mongo.py) is a script to migrate the data entries from MEF-ELine's namespaces from `storehouse` to MongoDB.
...
- Parametrize the environment variable `CMD` command and execute `2022.2.0/000_storehouse_to_mongo.py` script (the command is passed via an env var to avoid conflicts with `kytosd`, since depending how you set the `PYTHONPATH` it can interfere)
...
```
CMD=load_evcs python3 scripts/db/2022.2.0/000_storehouse_to_mongo.py
```

And then, to insert (or update) the EVCs:

```
CMD=insert_evcs python3 scripts/db/2022.2.0/000_storehouse_to_mongo.py
```